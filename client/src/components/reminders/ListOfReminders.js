...
class ListOfReminders extends React.Component {
  ...
  componentDidMount() {
    axios.get("/api/reminders").then((resp) => {
      this.setState({
        // sort reminders by date
        reminders: resp.data.sort((a, b) =>
          a.reminderDate < b.reminderDate ? -1 : 1
        ),
        loading: false,
      });
    });
  ...
  render() {
    return (
      <div className="list-main-container">
        ...
        {!this.state.loading && this.state.reminders.length === 0 ? (
          <div>
            <h4>No reminders yet!</h4>
            <img
              src="/images/calendar.png"
              alt="calender icon"
              className="transparent-icon mt-3 mb-4"
            />
          </div>
        ) : (
          this.state.reminders.map((reminder, key) => {
            ...
            return (
              <div
                className="list-item shadow p-3 mb-4 bg-body rounded"
                key={reminder._id}
              >
                <Link to={"/reminders/" + reminder._id}>
                  <div className="list-item-innerbox">
                    <div className="list-item-img-container">
                      <img src={typeOfCareIcon} alt="reminder icon" />
                    </div>
